...
struct LedState {
    ...
    static constexpr const char* Name = "led.enabled";
};
...
struct UnnamedState {
    ...
    static constexpr StateTypeId Id = 3;
};
...
int main() {
    ...
    const auto first = Device(1);
    const auto second = Device(2);
    ...
    assert(manager.SetAvailability(first, RemoteDeviceAvailability::Connected));
    assert(manager.SetAvailability(second, RemoteDeviceAvailability::Stale));
    ...
    assert(StateIntrospection<Contract>::TryGetTypeId("counter.value", typeId));
    assert(typeId == CounterState::Id);
    assert(!StateIntrospection<Contract>::TryGetTypeId("missing", typeId));
    assert(typeId == 0);

    std::size_t deviceCount = 0;
    manager.ForEachDevice([&](const RemoteDeviceSnapshot&) {
        ++deviceCount;
    });
    ...
    assert(counter.Name != nullptr);
    assert(counter.State.HasValue);
    assert(counter.State.Value == 42);
    assert(counter.State.Availability == RemoteDeviceAvailability::Connected);

    std::size_t firstStateCount = 0;
    StateIntrospection<Contract>::ForEachState(
        manager,
        first,
        [&](const auto&) { ++firstStateCount; }
    );
    assert(firstStateCount == 2);

    std::size_t remoteStateCount = 0;
    StateIntrospection<Contract>::ForEachRemoteState(
        manager,
        [&](const auto&) { ++remoteStateCount; }
    );
    assert(remoteStateCount == 3);

    SerializedRemoteState<CounterState> serializedCounter;
    assert(StateSerialization<Contract>::Serialize<CounterState>(
        manager,
        first,
        serializedCounter
    ));
    assert(serializedCounter.TypeId == CounterState::Id);
    assert(serializedCounter.Revision == 1);
    assert(serializedCounter.PayloadSize == sizeof(int32_t));

    int32_t decodedCounter = 0;
    assert(StateCodec<CounterState>::Decode(
        serializedCounter.Payload.data(),
        serializedCounter.PayloadSize,
        decodedCounter
    ));
    assert(decodedCounter == 42);

    bool runtimeSelected = false;
    assert(StateSerialization<Contract>::Serialize(
        manager,
        second,
        UnnamedState::Id,
        [&](const auto& record) {
            ...
            if constexpr (std::is_same_v<Record, SerializedRemoteState<UnnamedState>>) {
                ...
                assert(StateCodec<UnnamedState>::Decode(
                    record.Payload.data(),
                    record.PayloadSize,
                    value
                ));
    ...
    StateSerialization<Contract>::ForEachRemoteState(
        manager,
        [&](const auto& record) {
            assert(record.PayloadSize <= record.Payload.size());
            ++serializedCount;
        }
    );
